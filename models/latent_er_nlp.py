...
class LatentERNLP(ContinualModel):
    """
    ...
    def __init__(self, backbone, loss, args, transform):
        super(LatentERNLP, self).__init__(backbone, loss, args, transform)
    ...
    def begin_task(self, dataset):
        ...
        if self.current_task == 0 or len(self.candidate_feature_layers) == 1 or not self.auto_layer:
            ...
            if toss > 0.7:
                ...
                selected_feature_layer = torch.where(acc > self.threshold, torch.tensor(True).to(self.device), torch.tensor(False).to(self.device))
                ...
                if len(candidate_feature_layers) == 0 and len(candidate_feature_layers) > 3:
    ...
    def _evaluate_layer(self, probing_batch_num: int, dataset):
        ...
        for i, (encs, ls) in enumerate(zip(encoding_list, label_list)):
            ...
            acc_ = torch.where(predict == ls, torch.tensor(1).to(self.device), torch.tensor(0).to(self.device))  # example_size, feature_layer_num
    ...
    def observe(self, inputs, inputs_mask, labels, labels_name=None, labels_mask=None, task_labels=None):
        ...
        if not self.buffer.is_empty() and self.current_task > 0:
            ...
            (m_examples, m_examples_mask, m_labels, m_features), choice1 = self.buffer.get_data(
                self.args.minibatch_size)
            latent_features = self.net.encoder_adaptor(m_features)
            latent_outputs = self.net.classify(latent_features)
            loss += self.args.beta * self.loss(latent_outputs, m_labels)
        ...
        # todo:
        encoding = self.net.encoder(inputs, attention_mask=inputs_mask,
