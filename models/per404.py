...
class Per404(ContinualModel):
    """
    feature: with layer-wise regularization; start from instance
    """
    NAME = 'per404'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']
    
    def __init__(self, backbone, loss, args, transform=None):
        super(Per404, self).__init__(backbone, loss, args, transform)
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        if not self.buffer.is_empty():
            # begin: Loss 2
            # todo：buffer.get_data
            # todo：obtain a batch of data from buffer
