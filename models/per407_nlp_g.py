...
class Per407NLPg(ContinualModel):
    """
    feature: with features + logits regularization, without pseudo replay; start from instance
    """
    NAME = 'per407nlpg'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']
    
    def __init__(self, backbone, loss, args, transform):
        super(Per407NLPg, self).__init__(backbone, loss, args, transform)
    ...
    def observe(self, inputs, inputs_mask, labels, labels_name=None, labels_mask=None, task_labels=None):
        ...
        if not self.buffer.is_empty():
            ...
            (m_examples, m_examples_mask, m_labels, m_features), choice1 = self.buffer.get_data(
                self.args.minibatch_size)
            # buf_outputs =
            buf_feat_gen = self.net.layer_wise_forward(m_examples, m_examples_mask)
        ...
        # todo:
        features = self.net.layer_wise_forward(inputs, inputs_mask)
