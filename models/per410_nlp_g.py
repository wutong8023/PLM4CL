...
class Per410NLPg(ContinualModel):
    """
    feature: with logits regularization; start from instance ; update features immediately.
    """
    NAME = 'per410nlpg'
    COMPATIBILITY = ['class-il', 'domain-il', 'task-il', 'general-continual']
    
    def __init__(self, backbone, loss, args, transform):
        super(Per410NLPg, self).__init__(backbone, loss, args, transform)
        
        self.buffer = PERBufferNLP(self.args.buffer_size, self.device, require_label_name=False,
                                   mode="reservoir_imbalance")
    ...
    def observe(self, inputs, inputs_mask, labels, labels_name=None, labels_mask=None, task_labels=None):
        ...
        if not self.buffer.is_empty():
            ...
            self.buffer.update_batch_features(m_examples, m_labels, buf_outputs, choice)
            # end: Loss 2
