...
import torch
from utils.per_buffer_limited import PERBufferLimited
...
class Per501g(ContinualModel):
    ...
    def __init__(self, backbone, loss, args, transform):
        super(Per501g, self).__init__(backbone, loss, args, transform)
        
        self.buffer = PERBufferLimited(self.args.buffer_size, self.device)
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        if not self.buffer.is_empty():
            # begin: Loss 2
            (buf_inputs, buf_labels, buf_features), choice = self.buffer.get_data(
            ...
            #     self.args.minibatch_size, transform=self.transform)
            buf_feat_gen = self.net.layer_wise_forward(buf_inputs)
            loss += self.args.alpha * F.mse_loss(buf_feat_gen, buf_features)
            # end: Loss 3
            ...
            for i in range(self.args.pseudo_size):
                ...
                loss_temp += self.args.beta * self.loss(pseudo_outputs, buf_labels)
