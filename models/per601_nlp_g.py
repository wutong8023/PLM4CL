...
def get_parser() -> ArgumentParser:
    ...
    parser.add_argument('--gamma', type=float, required=True,
                        help='the weight for proto update')
    ...
    parser.add_argument('--require_label_name', type=bool, required=False, default=False,
                        help='the number of pseudo instances')
    parser.add_argument('--require_proto', type=bool, required=False, default=True,
                        help='the number of pseudo instances')
    return parser


class Per601NLPg(ContinualModel):
    """
    ...
    def __init__(self, backbone, loss, args, transform):
        super(Per601NLPg, self).__init__(backbone, loss, args, transform)
    ...
    def observe(self, inputs, inputs_mask, labels, labels_name=None, labels_mask=None, task_labels=None):
        ...
        if not self.buffer.is_empty():
            ...
            (m_examples, m_examples_mask, m_labels, m_features), choice = self.buffer.get_data(
                self.args.minibatch_size)
            buf_feat_gen = self.net.features(m_examples, m_examples_mask, proto)
            loss += self.args.alpha * F.mse_loss(buf_feat_gen, m_features)
            # update representation
            self.buffer.update_batch_features(m_examples, m_labels, buf_feat_gen, choice)
            ...
            # update representation
            self.buffer.update_batch_features(m_examples, m_examples_mask, m_labels, buf_feat_gen, choice)
            ...
            for i in range(self.args.pseudo_size):
                ...
                pseudo_outputs = self.net.classify(pseudo_feature, proto)
